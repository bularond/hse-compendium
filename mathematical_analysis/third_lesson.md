...
1. $\{x_n\}, \{y_n\}$ - б.м. ограниченная $\implies \{x_n \cdot y_n\}$ - б. м.

**Доказательство**\
1. 
$$
\Epsilon > 0 \exist N_1 \forall n \ge N_1 \implies |x_n| < \Epsilon / 2 \\
\exist N_2 \forall n > N_2 \implies |y_n| < \Epsilon / 2 \\
...
\sqrt[k]{a} > 1; \frac{n}{(\sqrt[k]{0})^n} \xrightarrow[]{n\to\infin} 0 \implies 
\left(\frac{n}{(\sqrt[k]{a})^n}\right)^k \xrightarrow[]{n\to\infin} 0
$$

## Арифметические операции над сходящимися последовательностями
**Утверждение 6**\
Если $\underset{n\to\infin}{\lim}x_n = a; \underset{n\to\infin}{\lim}x_n = a$, то
...
\end{rcases}
x_n \pm y_n = (a + b) + (\alpha_n + \beta_n); \text{т.е. } x_n + y_n \xrightarrow[n\to\infin]{}
a + b
$$

2. 
$$
x_n \cdot y_n = (a + \alpha_n) (b + \beta_n) = (a \cdot b) + a \beta_n + b \alpha_n, \text{т.е.}
...
\text{следовательно } \forall y_n \implies \left|\frac{1}{y_n}\right| < M
$$
...
\left|\frac{b-y_n}{y_n \cdot b}\right| = \frac{1}{b\cdot y_n} |b-y_n|
$$
...
**Докозательство**
$$
1. \Epsilon > 0; \sigma = \frac{1}{\Epsilon} > 0 \exist N \forall n \ge N \implies
...
Последовательность $x_1 < x_2 < \dots < x_n < \dots$ т.е. $x_n < x_{n+1} (n=1,2 \dots)$ - возрастет\
...
**Теорема**\
Если последовательность $\{x_n\}$ ну убывает и ограничена сверху, то она сходится
...
< 1 + \sum_{k=1}^{n} \frac{1}{k!} < 1 + 11 + \frac{1}{2} + \frac{1}{2^2} + \dots + 
\frac{1}{2^{k-1}} = 1 + \frac{1 - (\frac{1}{2})^2}{1 - \frac{1}{2}} < 1 + \frac{1}{1/2} = 3
$$
$$
k! = 1 \cdots 2 \cdot 3 \dots k \\
2^{k-2} = 2 \cdot 2 \dots 2 \\
2^{k-1} < k! \\
